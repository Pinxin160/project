...
from user_manager import login, load_users, save_users
...
class ChessGame:
    def __init__(self, root, piece_ranks, user_file, saved_last_game=None):
        ...
        self.game_window.protocol("WM_DELETE_WINDOW", self.on_close) # 當遊戲視窗被關閉時，關閉主視窗
    ...
    def draw_chessboard(self):
        ...
        for row in range(4):
            for col in range(8):
                ...
                x2 = x1 + square_width
                y2 = y1 + square_height
                self.canvas.create_rectangle(x1, y1, x2, y2, fill="#f5f5f5", outline="#000")
    ...
    def load_game_state(self, file_name):
        ...
        try:
            ...
            try: # 刪除被竄改的檔案
                os.remove(file_name)
                messagebox.showinfo("遊戲繼續", f"遊戲紀錄被竄改! 已刪除：{file_name}")
    ...
    def check_game_over(self):
        ...
        self.game_over = True
        self.user_file['current_score'] = player_score
    ...
    def handle_click(self, x, y):
        ...
        if self.selected_piece_id:
            ...
            if tuple(self.pieces[self.selected_piece_id]['position']) == (x, y):
                self.selected_piece_id = None
...
class ChessGame_2p:
    def __init__(self, root, piece_ranks, user_file, second_user_file=None):
        self.root = root
        self.game_window = tk.Toplevel(self.root)  # 創建第二個視窗（遊戲視窗）
        self.game_window.title("Chinese Dark Chess Game")
        self.piece_ranks = piece_ranks
        self.user_file = user_file
        self.user_account = user_file['username']
        self.second_user_file = second_user_file
        self.second_user_account = second_user_file['username']
    ...
    def initialize_game_state(self):
        """Initialize the chess pieces and place them on the board."""
        self.pieces = {}
        self.death_pieces_player1 = {}
        self.death_pieces_player2 = {}
        self.player1_color = None  # Player1's color
        self.player2_color = None  # Player2's color

        self.current_turn = self.user_account 
        self.game_over = False
        self.no_capture = 0
        self.selected_piece_id = None
        self.cnt = 0
    ...
    def check_game_over(self):
        ...
        if self.no_capture >= 50:
            ...
            player_score1 = 0
            player_score2 = 0
            message = f"雙方進行翻棋或移動棋子連續達 50 次: {result}，兩位玩家得分皆為 0 。"
        elif len(self.death_pieces_player2) == 16:
            result = f"{self.user_account} 玩家獲勝"
            player_score1 = 500 + 100 * (16 - len(self.death_pieces_player1))
            player_score2 = 0
            message = f"{result}！得分為 {player_score1} 。"
        elif len(self.death_pieces_player1) == 16:
            result = f"{self.second_user_account} 玩家獲勝"
            player_score1 = 0
            player_score2 = 500 + 100 * (16 - len(self.death_pieces_player2))
            message = f"{result}！得分為 {player_score2} 。"
        else:
            return  # 若未達結束條件，直接返回

        self.game_over = True
        self.user_file['current_score'] = player_score1
        self.second_user_file['current_score'] = player_score2
        messagebox.showinfo("遊戲結束", message)
    ...
    def handle_click(self, x, y):
        """Handle clicks on pieces."""
        print(f"現在輪到: {self.current_turn}")
        current_player_color = self.player1_color if self.current_turn == self.user_account else self.player2_color
        x, y = self.closest_square_center((x, y))
        ...
        if self.selected_piece_id:
            ...
            if not selected_piece_info:
                print(f"點擊位置 ({x}, {y}) 沒有棋子。")
                return
            if selected_piece_info["reveal"] and selected_piece_info["color"] != current_player_color: # 如果該棋子已揭示但不是玩家的棋子，請玩家重選一顆
                print(f"選擇了棋子: {selected_piece_info['name']}，位置: {selected_piece_info['position']}")
                self.selected_piece_id = None
                self.prompt_invalid_piece()
                return
            if selected_piece_info and selected_piece_info["reveal"] and selected_piece_info["color"] == current_player_color: # 如果該棋子已揭示並且是玩家的棋子，則選擇該棋子
                return
            elif not selected_piece_info["reveal"]: # 如果該棋子未揭示，執行揭示操作
                self.reveal_piece(self.selected_piece_id)
                print(f"在 ({x}, {y}) 翻開了一顆棋子: {self.pieces[self.selected_piece_id]['name']}")
                if self.player1_color is None: # 如果是第一次揭示棋子時，設定玩家與電腦的顏色
    ...
    def perform_capture(self, from_piece_id, target_piece_id):
        ...
        self.canvas.delete(self.pieces[target_piece_id]["canvas_id"])
        self.canvas.delete(self.pieces[target_piece_id]["text_id"])
        if self.pieces[target_piece_id]['color'] == self.player1_color:
            self.death_pieces_player1[target_piece_id] = self.pieces[target_piece_id]  
        else:
            self.death_pieces_player2[target_piece_id] = self.pieces[target_piece_id]
        del self.pieces[target_piece_id]  # 更新棋盤，移除被吃的棋子
    ...
    def switch_turn(self):
        self.selected_piece_id = None
        self.cnt = 0
        if self.current_turn == self.user_account:
            self.current_turn = self.second_user_account
            print("============================================")
            print("現在回合: ", self.current_turn)
            self.turn_label.config(text=f"目前輪到: {self.current_turn}")
        else:
...
def main(user_file, saved_last_game):
    ...
    def on_start_game_2p():
        ...
        # 創建一個新視窗讓第二位玩家輸入帳號和密碼
        login2 = tk.Toplevel(root)
        login2.title("雙人遊戲 - 第二位玩家登入")
        login2.geometry("400x400")
        login2.configure(bg="#f5f5f5")
        tk.Label(login2, text="帳號", font=("Arial", 14)).grid(row=1, column=0, padx=10, pady=5)
        username_entry = tk.Entry(login2, font=("Arial", 12))
        username_entry.grid(row=1, column=1, padx=10, pady=5)
        tk.Label(login2, text="密碼", font=("Arial", 14)).grid(row=2, column=0, padx=10, pady=5)
        password_entry = tk.Entry(login2, show="*", font=("Arial", 12))
        password_entry.grid(row=2, column=1, padx=10, pady=5)
        confirm_button = tk.Button(login2, text="登入", command=login_for_2p, font=("Arial", 14))
    ...
    # 添加按鈕
    start_button = tk.Button(root, text="單人遊戲", command=on_start_game, font=("Arial", 16))
    start_button.pack(expand=True)
    start_button_2p = tk.Button(root, text="雙人遊戲", command=on_start_game_2p, font=("Arial", 16))
    start_button_2p.pack(expand=True)
    root.mainloop()

    if user2_file:
        users = load_users()
        for user in users["users"]:
            if user["username"] == user2_file["username"]:
                user["chess_2p_score"] = user2_file["current_score"]
                break
        save_users(users)
    
    return user_file["current_score"]



# if __name__ == "__main__":
#     user_account = {"username": "123", "password": "123", "level": 1, "score": 0} # 當時登入的使用者~~~
